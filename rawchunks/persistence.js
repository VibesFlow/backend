...
class PersistenceService {
  ...
  transformToFilCDNFormat(item) {
    ...
    return {
      ...
      rta_duration: this.formatDuration(item.total_duration || 0), // Sum of chunk DURATIONS, not sizes
      ...
      chunks_detail: sortedChunks.map(chunk => ({
        ...
        cid: chunk.cid,
        size: chunk.size,
        root_id: chunk.root_id, // FIXED: Add root_id (this is NOT rta_id)
        url: `https://${filecoinAddress}.calibration.filcdn.io/${chunk.cid}`,
        duration: chunk.duration || 60, // Most are 60s, but final chunk may be shorter
        participants: chunk.participants || 1,
        owner: `${item.creator}.testnet`, // FIXED: Add owner field with .testnet
